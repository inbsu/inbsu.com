...
test("server-renders Yu Hang's personal journal", async () => {
  ...
  assert.doesNotMatch(html, /https:\/\/(?:x\.com\/yuinbsu|www\.youtube\.com\/@inbsu|github\.com\/inbsu)/);
  assert.doesNotMatch(html, /X · @yuinbsu|YouTube · @inbsu|GitHub · @inbsu/);
  ...
  assert.doesNotMatch(html, /class="about-elsewhere"|id="elsewhere-title"|>ELSEWHERE</);
